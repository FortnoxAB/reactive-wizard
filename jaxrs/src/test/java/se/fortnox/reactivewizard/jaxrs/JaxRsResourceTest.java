...
public class JaxRsResourceTest {
	...
	public void shouldSupportSimpleHeaderParamTypes() {
		assertThat(body(JaxRsTestUtil.getWithHeaders(service,
				...
				new HashMap<String,List<String>>(){{put("myHeader", asList("accepts"));}}
		))).isEqualTo("\"header: accepts\"");

		assertThat(body(JaxRsTestUtil.getWithHeaders(service,
				"/test/acceptsHeaderInteger",
				new HashMap<String,List<String>>(){{put("myHeader", asList("4"));}}
				))).isEqualTo("\"header: 4\"");
		assertThat(body(JaxRsTestUtil.getWithHeaders(service, "/test/acceptsHeaderEnum",
				new HashMap<String,List<String>>(){{put("myHeader", asList("ONE"));}}
	...
	public void shouldSupportMissingHeaderParams() {
		assertThat(body(JaxRsTestUtil.getWithHeaders(service,
				"/test/acceptsHeaderString",
				new HashMap<String,List<String>>(){{put("dummy", asList("accepts"));}}
	...
	public void shouldReturnErrorForBadJson() {
		assertBadRequest(post(service, "/test/jsonParam", "{hej}"),
				"{\"id\":\".*\",\"error\":\"invalidjson\",\"message\":\"Unexpected character ('h' (code 104)): was expecting double-quote to start field name\"}");
		assertBadRequest(post(service, "/test/jsonParam", "{\"age\": \"nan\"}"),
				"{\"id\":\".*\",\"error\":\"invalidjson\",\"message\":\"Can not deserialize value of type int from String \\\\\"nan\\\\\": not a valid Integer value\"}");
		assertBadRequest(post(service, "/test/jsonParam", (String)null),
	...
	public void shouldSupportUUIDAsHeader() {
		...
		assertThat(body(JaxRsTestUtil.getWithHeaders(service,
				...
				new HashMap<String,List<String>>(){{put("id", asList(uuid.toString()));}}
