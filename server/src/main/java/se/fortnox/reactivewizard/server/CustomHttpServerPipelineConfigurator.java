...
/**
 * Adds custom filters to the pipeline.
 *
 * Currently only adds a fix for 204 NoContent and 1XX responses, which should have neither body nor Content-Length
 ...
 */
public class CustomHttpServerPipelineConfigurator implements Action1<ChannelPipeline> {

    private final int maxInitialLineLength;
    private final int maxChunkSize;
    private final int maxHeaderSize;

    public CustomHttpServerPipelineConfigurator(int maxInitialLineLength, int maxChunkSize, int maxHeaderSize) {
    ...
    public void call(ChannelPipeline pipeline) {
        pipeline.replace(HttpHandlerNames.HttpServerDecoder.getName(), HttpHandlerNames.HttpServerDecoder.getName(), new HttpRequestDecoder(maxInitialLineLength, maxHeaderSize, maxChunkSize));
